...
# PR 12: Windows real impls (EnvProvider, Paths, FileLock, IpcServer/Client, Authority, FsPermissions, Windows-internal caller resolution)

**Size:** L · **Touches:** `boxpilot-platform/src/windows/{env,lock,ipc,authority,fs_perms}.rs`. **Linux non-regression:** zero — pure Windows-side additions.

This PR fills in the Windows impls that AC5 ("Windows minimum boot") needs. Stub impls (Service, Trust, Active, Logs, Bundle, Version, UserLookup, CoreArchive, FsMetadata) stay `unimplemented!()` per the spec §5 trait inventory; real impls arrive in Sub-project #2.

## Task 12.1: Windows Authority — `AlwaysAllow` (per COQ3)

**Files:**
- Modify: `crates/boxpilot-platform/src/windows/authority.rs` (the file may not exist yet — create)
...
- [ ] **Step 2: Wire into `windows/mod.rs`**

```rust
pub mod authority;
```

- [ ] **Step 3: Run cross-target tests**

Run: `cargo test -p boxpilot-platform`
Expected: green on Linux; `cargo check --target x86_64-pc-windows-gnu -p boxpilot-platform` clean.

- [ ] **Step 4: Commit**

```bash
git add crates/boxpilot-platform/src/windows
git commit -m "feat(platform/windows): AlwaysAllowAuthority (per COQ3)"
```

---

## Task 12.2: Windows Named Pipe IpcServer + IpcClient (real)

**Files:**
- Modify: `crates/boxpilot-platform/src/windows/ipc.rs`

This is the meatiest Windows-side code in Sub-project #1. Implements the §5.4.1 wire format end-to-end.

- [ ] **Step 1: Write the IpcServer**
...
async fn handle_connection(
    ...
    pipe.read_exact(&mut header).await.map_err(|e| HelperError::Ipc {
    ...
    pipe.read_exact(&mut body).await.map_err(|e| HelperError::Ipc {
    ...
    let aux = if aux_present {
        ...
        loop {
            ...
            pipe.read_exact(&mut len_buf).await.map_err(|e| HelperError::Ipc {
            ...
            pipe.read_exact(&mut acc[start..]).await.map_err(|e| HelperError::Ipc {
    ...
    pipe.write_all(&status.to_le_bytes()).await.map_err(|e| HelperError::Ipc {
        message: format!("write status: {e}"),
    })?;
    let body_len_u64 = body.len() as u64;
    pipe.write_all(&body_len_u64.to_le_bytes()).await.map_err(|e| HelperError::Ipc {
...
fn resolve_caller_sid(pipe_handle: std::os::windows::io::RawHandle) -> Result<CallerPrincipal, HelperError> {
    ...
    unsafe {
        let mut pid: u32 = 0;
        if GetNamedPipeClientProcessId(pipe_handle as isize, &mut pid) == 0 {
            return Err(HelperError::Ipc {
...
- [ ] **Step 2: Write the matching IpcClient (used by `boxpilotctl` in PR 14b)**
...
impl IpcClient for NamedPipeIpcClient {
    async fn call(
        ...
        pipe.write_all(&header).await.map_err(|e| HelperError::Ipc {
        ...
        pipe.read_exact(&mut status_buf).await.map_err(|e| HelperError::Ipc {
            message: format!("read status: {e}"),
        })?;
        let status = u32::from_le_bytes(status_buf);
        let mut body_len_buf = [0u8; 8];
        pipe.read_exact(&mut body_len_buf).await.map_err(|e| HelperError::Ipc {
            message: format!("read resp body_len: {e}"),
        })?;
        let resp_len = u64::from_le_bytes(body_len_buf) as usize;
        let mut resp_body = vec![0u8; resp_len];
        pipe.read_exact(&mut resp_body).await.map_err(|e| HelperError::Ipc {
...
- [ ] **Step 3: Wire `windows/mod.rs`**

```rust
pub mod ipc;
```

- [ ] **Step 4: Build for Windows GNU**

Run: `cargo build --target x86_64-pc-windows-gnu -p boxpilot-platform`
Expected: clean.
...
```bash
git add crates/boxpilot-platform/src/windows
git commit -m "feat(platform/windows): NamedPipeIpcServer + Client over §5.4.1 wire format"
```

---

## Task 12.3: Windows FileLock impl is already real (PR 6) — verify

PR 6 already shipped the `LockFileEx`-based Windows impl. This task just verifies the build still works.

- [ ] **Step 1: Verify**

Run: `cargo check --target x86_64-pc-windows-gnu -p boxpilot-platform`
Expected: clean.
...
## Task 12.4: Stub Windows impls for traits not needed by AC5

For each remaining trait without a Windows impl yet (`fs_meta`, `version`, `user_lookup`, `service`, `trust`, `active`, `logs`, `core_assets`), the file already has its `unimplemented!()` stub from PRs 3-9. This task confirms.

- [ ] **Step 1: Audit**

Run: `for f in crates/boxpilot-platform/src/windows/*.rs; do echo "=== $f ==="; grep -c 'NotImplemented\|unimplemented!' "$f"; done`

Each module that's not real (env, lock, ipc, authority, fs_perms — these ARE real) should report at least one `NotImplemented`/`unimplemented!()` reference.
...
```bash
cargo test --workspace                                                    # Linux non-regression
cargo check --target x86_64-pc-windows-gnu --workspace                    # required gate (since PR 11a)
cargo build --target x86_64-pc-windows-gnu -p boxpilot-platform           # builds without errors
```
