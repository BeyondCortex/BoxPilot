//! `service.stop` handler — mutating; controller required.
...
pub async fn handle(
    ...
    if !aux.is_none() {
        return Err(HelperError::Ipc {
            message: "service.stop takes no aux stream".into(),
        });
    }
    let _call = dispatch::authorize(&ctx, &principal, HelperMethod::ServiceStop).await?;
