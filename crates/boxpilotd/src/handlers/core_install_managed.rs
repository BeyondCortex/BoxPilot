//! `core.install_managed` handler — mutating; may claim controller.
...
pub async fn handle(
    ...
    if !aux.is_none() {
        return Err(HelperError::Ipc {
            message: "core.install_managed takes no aux stream".into(),
        });
    }
    ...
    let call = dispatch::authorize(&ctx, &principal, HelperMethod::CoreInstallManaged).await?;
