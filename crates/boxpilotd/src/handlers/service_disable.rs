//! `service.disable` handler — mutating; controller required.
...
pub async fn handle(
    ...
    if !aux.is_none() {
        return Err(HelperError::Ipc {
            message: "service.disable takes no aux stream".into(),
        });
    }
    let _call = dispatch::authorize(&ctx, &principal, HelperMethod::ServiceDisable).await?;
