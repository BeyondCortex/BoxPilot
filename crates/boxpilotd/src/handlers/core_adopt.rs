//! `core.adopt` handler — mutating; may claim controller.
...
pub async fn handle(
    ...
    if !aux.is_none() {
        return Err(HelperError::Ipc {
            message: "core.adopt takes no aux stream".into(),
        });
    }
    ...
    let call = dispatch::authorize(&ctx, &principal, HelperMethod::CoreAdopt).await?;
    ...
    let resp = crate::core::adopt::adopt(&req, &deps, controller).await?;
